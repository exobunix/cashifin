...
export interface ICalculatePriceInput {
  modelId: string;
  variantIndex?: number; // Index in the variants array
  answers: IUserAnswer[];
  region?: string;
  distanceKm?: number;
}
...
export interface IDeductionBreakdown {
  name: string;
  type: 'option_deduction' | 'advanced_rule' | 'multiplier' | 'logistics';
...
export interface IPriceEstimationResult {
  ...
  deductions: IDeductionBreakdown[];
}

export const calculateEstimatedPrice = async (input: ICalculatePriceInput): Promise<IPriceEstimationResult> => {
  ...
  const deductions: IDeductionBreakdown[] = [];
  ...
  const questions = await InspectionQuestion.find({ _id: { $in: questionIds } });
  ...
  questions.forEach((question) => {
    ...
    const matchedOption = question.options.find(
      (opt) => opt.optionText.toLowerCase() === selectedOptionText.toLowerCase()
    );
  ...
  activeRules.forEach((rule) => {
    ...
    if (rule.conditions && rule.conditions.length > 0) {
      const matchResults = rule.conditions.map((cond) => {
        ...
        if (valueInScope === undefined) return false;
